...
public class SportsShopDaoImp implements SportsShopDao {
	...
	try (final Connection connection = CONNECTION.getConnection(); final PreparedStatement preparedStatement = connection.prepareStatement(INSERT_PRODUCT);) {
	...
	try (final Connection connection = CONNECTION.getConnection(); final PreparedStatement preparedStatement = connection.prepareStatement(UPDATE_PRODUCT);) {
	...
	try (final Connection connection = CONNECTION.getConnection(); final PreparedStatement preparedStatement = connection.prepareStatement(DELETE_PRODUCT);) {
	...
	try (final Connection connection = CONNECTION.getConnection(); final PreparedStatement preparedStatement = connection.prepareStatement(SELECT_ALL_PRODUCT);) {
	    preparedStatement.setBoolean(1, false);
	    final ResultSet rs = preparedStatement.executeQuery();

	    while (rs.next()) {
		final Product product = new Product(rs.getString("name"), rs.getString("brand"), rs.getDouble("price"), rs.getString("size").charAt(0), 
